...
const Signup = () => {
  ...
  const [pic, setPic] = useState(""); // optional
    ...
    try {
      ...
      const { data } = await axios.post(
        "/api/user",
        {
          ...
          email,
          password,
          // ✅ if no pic uploaded, backend will handle empty string
          pic: pic || undefined,
        },
        config
      );
  ...
  const postDetails = (pics) => {
    setPicLoading(true);
    if (!pics) {
      // ✅ now we just skip instead of forcing selection
      setPicLoading(false);
      return;
    }
